...
namespace glass {
    ...
    // the hairline and the top light both come from the live palette, on a light page
    // a white rim would vanish
    inline ImU32 border() { return theme::at().border; }
    inline int   rim()    { return theme::at().rim; }
    ...
    inline void taper(ImDrawList* dl, const ImVec2* pts, const ImVec2* offs,
        ...
        for (int i = 0; i < n; i++) {
            ImU32 c = theme::at().rim_dark
                    ? IM_COL32(0, 0, 0, (int)(rim() * alpha[i]))
                    : IM_COL32(255, 255, 255, (int)(rim() * alpha[i]));
    ...
    inline void cross(ImDrawList* dl, ImVec2 c, float arm, float half, float rd) {
        ...
        dl->PathStroke(border(), ImDrawFlags_Closed, hairline());
    }
